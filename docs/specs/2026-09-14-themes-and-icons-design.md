...
nodes:
  ...
  - id: fn
    icon: simple-icons/awslambda # brand set
...
- Participants are included provisionally: the implementation plan's first
  task verifies that D2 sequence actors render an `icon:`. If they do not,
  `participants[].icon` is dropped from the schema and this section is
  amended before any further task starts.
...
- Reject: `<script>`, `<foreignObject>`, any `on*` attribute, any `href`/
  `xlink:href` that is not a `#fragment`, `<style>` with `@import`, and DTD
  declarations.
...
- `icons.json` — `{ [name]: svg }`, sanitized and minified
...
fails if a set exceeds 5 MB so a bad upstream change cannot silently bloat
installs.
...
- `--icons <dir>` — repeatable; registers a local set directory. Accepted by
  `render`, `validate`, `describe`, `edit`, and `mcp`.
